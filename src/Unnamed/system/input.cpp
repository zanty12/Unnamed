...
{
    auto mousepos = XMFLOAT2(static_cast<float>(m_MousePos.x), static_cast<float>(m_MousePos.y));
...
XMVECTOR Input::ConvertNDCToWorld(XMFLOAT2 ndc, XMMATRIX viewMatrix,
                                  XMMATRIX projectionMatrix)
{
    XMMATRIX invViewProj = XMMatrixInverse(nullptr, viewMatrix * projectionMatrix);

    XMVECTOR ndcPos = XMVectorSet(ndc.x, ndc.y, 1.0f, 1.0f);
...
XMFLOAT2 Input::ConvertScreenToNDC(XMFLOAT2 screenPos, int screenWidth, int screenHeight)
{
    XMFLOAT2 ndc;
