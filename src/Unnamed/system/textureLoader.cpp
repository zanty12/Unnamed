...
ID3D11Texture2D* TextureLoader::LoadTexture(const std::wstring& path)
{
    ...
    HRESULT hr = LoadFromWICFile(path.c_str(), WIC_FLAGS_NONE, &metadata, scratch_image);
    ...
    Renderer::GetDeviceContext()->UpdateSubresource(texture2D, 0, nullptr, scratch_image.GetImages()->pixels,
