...
#include "manager.h"
#include "entity.h"
#include "CScreenStencil.h"

#include "components/CSprite2D.h"

void CScreenStencil::Start()
{
    //Read texture
    //texture_->Start();
...
void CScreenStencil::Draw()
{
    ...
    vertex[0].Diffuse = DirectX::XMFLOAT4(0.2f, 0.2f, 0.2f, 1.0f);
    vertex[0].TexCoord = DirectX::XMFLOAT2(start_uv_.x, start_uv_.y);

    vertex[1].Position = DirectX::XMFLOAT3(SCREEN_WIDTH, 0.0f, 0.0f);
    vertex[1].Normal = DirectX::XMFLOAT3(0.0f, 0.0f, 0.0f);
    vertex[1].Diffuse = DirectX::XMFLOAT4(0.2f, 0.2f, 0.2f, 1.0f);
    vertex[1].TexCoord = DirectX::XMFLOAT2(end_uv_.x, start_uv_.y);

    vertex[2].Position = DirectX::XMFLOAT3(0.0f, SCREEN_HEIGHT, 0.0f);
    vertex[2].Normal = DirectX::XMFLOAT3(0.0f, 0.0f, 0.0f);
    vertex[2].Diffuse = DirectX::XMFLOAT4(0.2f, 0.2f, 0.2f, 1.0f);
    vertex[2].TexCoord = DirectX::XMFLOAT2(start_uv_.x, end_uv_.y);

    vertex[3].Position = DirectX::XMFLOAT3(SCREEN_WIDTH, SCREEN_HEIGHT, 0.0f);
    vertex[3].Normal = DirectX::XMFLOAT3(0.0f, 0.0f, 0.0f);
    vertex[3].Diffuse = DirectX::XMFLOAT4(0.5f, 0.5f, 0.5f, 1.0f);
